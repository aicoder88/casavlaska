...
export default function RoiCalculator() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      yPos += 10;
      pdf.setFontSize(10);
      pdf.text(`Total Investment: €${results.totalCost.toLocaleString()}`, 20, yPos);
      yPos += 7;
      pdf.text(`Annual Net Income: €${results.annualNetIncome.toLocaleString()}`, 20, yPos);
      yPos += 7;
      pdf.text(`Net Yield: ${results.netYield.toFixed(1)}%`, 20, yPos);
      yPos += 7;
      pdf.text(`Payback Period: ${results.payback.toFixed(1)} years`, 20, yPos);
      
      // Add disclaimer
      yPos += 20;
      pdf.setFontSize(8);
      pdf.text('Generated by CasaVlaška Calculator. For informational purposes only.', 20, yPos);
      pdf.text('Contact: +385 99 343 3344 | drago@casavlaska.com', 20, yPos + 5);
